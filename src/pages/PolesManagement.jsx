...
// ===================================
// PAGE : GESTION DES PÔLES
// Permet de gérer les différents départements (EM, Instruction, Ops...)
// ===================================
export default function PolesManagement() {
    const { allPlayers, polesData, updatePoles } = useGame();

    // --- FONCTIONS UTILITAIRES ---

    // Récupérer les membres d'un pôle
    ...
    // Mise à jour générique d'un pôle
    ...
    // Basculer l'état d'un objectif (Fait / Pas fait)
    ...
    // Élément de liste pour un objectif
    ...
    // Carte Pôle (Format Mobile : Accordéon)
    const PoleCard = ({ poleKey }) => {
        ...
        // État d'ouverture (Plié par défaut sur mobile)
        const [isOpen, setIsOpen] = useState(true);

        useEffect(() => {
            // Détection Mobile basique (< 768px -> Tablette/Mobile)
        ...
        return (
            <div style={{ backgroundColor: '#0f172a', position: 'relative', zIndex: 10 }} className="bg-[#0f172a] border border-slate-700 rounded-xl overflow-hidden shadow-xl flex flex-col h-fit transition-all duration-300">
                {/* EN-TÊTE DE LA CARTE (CLIQUABLE) */}
                <div
                    className="bg-slate-900 p-4 border-b border-slate-700 flex justify-between items-start cursor-pointer hover:bg-slate-800 transition-colors"
                    onClick={() => setIsOpen(!isOpen)}
                >
                    <div className="flex-1">
                        <h3 className="text-lg font-bold text-white flex items-center gap-2">
                            <Briefcase size={20} className="text-blue-500" />
                            {pole.label || poleKey} {/* Support fallback */}
                        </h3>
                    ...
                    <div className="flex items-center gap-4">
                        {/* Sélecteur de Référent (Bloquer la propagation du clic) */}
                        <div className="flex flex-col items-end" onClick={(e) => e.stopPropagation()}>
                            ...
                            <select
                                ...
                                className="bg-slate-950 border border-slate-700 text-xs text-white rounded px-2 py-1 outline-none focus:border-amber-500 max-w-[120px] md:max-w-[150px]"
                            >
                ...
                {/* CONTENU (Masqué si fermé) */}
                {isOpen && (
                    <div className="p-4 space-y-6 flex-1 overflow-y-auto custom-scrollbar animate-in slide-in-from-top-2 duration-300">
